...
AMIError.prototype._proto_ = Error.prototype;
...
AMIParser.prototype._transform = function (chunk, encoding, done) {
  ...
  while (eom[eomIndex]) {
    ...
    while ((foundEom = tmpLocalBuffer.indexOf(eom[eomIndex])) != -1) {
      ...
      if (
        messajeJson["Response"] &&
        messajeJson["ActionID"] &&
        messajeJson["Event"]
      ) {
...
function AMI(hostname, port, username, password) {
  ...
  function run() {
    ...
    _parser.on("response", function (data) {
      if (
        data["ActionID"] &&
        _actions[data["ActionID"]] &&
        _actions[data["ActionID"]].cb
      ) {
    ...
    _parser.on("message", function (data) {
      if (data["Message"]) {
        me.emit("eventAny", data);
      }
    });
    ...
    _socket.on("connect", function () {
      me.action(
        "Login",
        { Username: username, Secret: password, Events: "On" },
        function (data) {
   ...
   */
  this.action = function (name, data, actionId = "", cb) {
    var name = name || "",
      data = data || {},
      dataJson = {},
      dataTxt = "",
